...
export const api = {
    ...
    inspect: (input: string, format?: string) =>
        request<{ tree: Record<string, unknown>; metadata: Record<string, unknown>; format: string }>('/inspect', {
            method: 'POST',
            body: JSON.stringify({ input, format }),
        }),
    ...
    createShare: (data: { title: string; content: string; format: string; description?: string; isPublic?: boolean; tags?: string[]; images?: string[] }) =>
    ...
    myShares: () =>
        request<Record<string, unknown>[]>('/share/my'),

    deleteShare: (id: string) =>
        request<{ deleted: boolean }>(`/share/${encodeURIComponent(id)}`, { method: 'DELETE' }),

    updateShare: (id: string, data: { title?: string; description?: string; content?: string; tags?: string[]; images?: string[] }) =>
        request<Record<string, unknown>>(`/share/${encodeURIComponent(id)}`, {
            method: 'PATCH',
            body: JSON.stringify(data),
        }),
    ...
    savedShares: () =>
        request<Record<string, unknown>[]>('/share/saved'),

    likedShares: () =>
        request<Record<string, unknown>[]>('/share/liked'),
    ...
    getMe: () =>
        request<{ userId: string; email: string; name: string | null; avatarUrl: string | null; provider: string; isAdmin?: boolean }>('/auth/me'),

    login: (data: { provider: string; email: string; providerId: string; name?: string; avatarUrl?: string }) =>
    ...
    getUser: (userId: string) =>
        request<{ id: string; name: string | null; avatarUrl: string | null; createdAt: string }>(`/auth/user/${encodeURIComponent(userId)}`),
